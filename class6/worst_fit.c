...
void *my_malloc(size_t size) {
  ...
  metadata_t *worst_fit_prev = NULL;
  ...
  while (metadata) {
    if(metadata->size >= size){
      if(count == 0){
        //最初だったらとりあえずworst_fitに設定
        worst_fit = metadata;
        worst_fit_prev = prev;
        count++;
      }
      //よりサイズが大きいものをworst_fitに
      if(worst_fit->size < metadata->size){
        worst_fit = metadata;
        worst_fit_prev = prev;
        //printf("%s\n", "worst");
      }
  ...
  remove_from_free_list(worst_fit, worst_fit_prev);
