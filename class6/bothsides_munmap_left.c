...
void add_to_free_list(metadata_t *metadata) {
  ...
  //left
  cur_heap = heap.free_head;
  prev = NULL;

  int flag = 1;

  while(cur_heap){
    void *ptr2 = cur_heap + 1;
    metadata_t *next_ptr = (metadata_t *)((char *)ptr2 + cur_heap->size);
    if(next_ptr == metadata){
      ...
      //munmap
      if(cur_heap->size == 4096 - sizeof(metadata_t) && (uintptr_t)(cur_heap) % 4096 == 0){
        flag = 0;
        if (prev) {
          prev->next = cur_heap->next;
        } else {
        ...
        munmap_to_system(cur_heap, 4096);
      }
      break;
    }
    prev = cur_heap;
