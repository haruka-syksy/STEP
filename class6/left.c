//解放しようとしている領域の左側がつなげられる場合(bestfit適用)
#include <assert.h>
#include <stdbool.h>
#include <stddef.h>
#include <stdint.h>
...
void add_to_free_list(metadata_t *metadata) {
  ...
  int flag = 1;//領域がつなげられるかの判断(つなげられたら0にする)

  metadata_t *cur_heap = heap.free_head;

  while(cur_heap){
    //cur_heapの次の領域の先頭のポインタを求める
    //
    // ... | metadata(cur_heap) | object | metadata | object | ...
    //     <---------- cur_heap --------->
    //     ^                    ^　　　　　^
    //     cur_heap             ptr      next_ptr ==  metadata
    void *ptr = cur_heap + 1;
    metadata_t *next_ptr = (metadata_t *)((char *)ptr + cur_heap->size);
    if(next_ptr == metadata){
      flag = 0;

      cur_heap->size = cur_heap->size+sizeof(metadata_t)+metadata->size;
